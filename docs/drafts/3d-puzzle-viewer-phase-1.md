...
3×3の基準造形は、中国製ステッカーレスキューブらしい印象を目標にする。各キュービーは、面ごとに色を変えられる丸い樹脂シェルとして描く。直線状のパネル辺と外周の角は控えめに丸め、面内の格子点に接するパネル角だけは大きく丸める。これにより、面の中央は平らに保ちながら、面内の格子点は隣り合う丸い四隅がつくる小さな星形の抜けになる。キュービー同士はほぼ接する細い隙間に抑え、中性の内部機構はその隙間からだけ見せる。太い黒線や色付きの縁、センターのロゴは使わない。
